...
In practice, storing the full retain set is prohibited under General Data Protection Regulation (GDPR) and California Consumer Privacy Act (CCPA) regulations for privacy-sensitive data, while for non-private data, retraining remains computationally expensive due to large-scale models and datasets. We therefore maintain a small replay buffer $D_r \subset D^\text{\textit{full}}_r$, comprising at least 10\% of $|D^\text{\textit{full}}_r|$, such that $|D_r| \ll |D^\text{\textit{full}}_r|$, where $D^\text{\textit{full}}_r$ represents full retaining data \footnote{From this point, $D_r$ denotes the experience replay buffer unless specified.}. The consequences of insufficient or absent buffer size are discussed in detail in Section~\ref{8.1}.
...
Here, \( \cancel{\rightarrow} \) denotes that the mapping no longer holds, meaning the model has forgotten the corresponding mapping, while \( \rightarrow \) indicates that the mapping still holds.
...
\begin{align}
    ...
    f_{\mathcal{M}^{(t)}} &: \textcolor{red}{\mathcal{X}_{D_{\text{new}}^{(t)}} \rightarrow \mathcal{Y}_{D_{\text{new}}^{(t)}}} \label{Eq:3c}
\end{align}
\end{subequations}
...
\begin{enumerate}
    ...
    \item Learning and unlearning occur simultaneously, and if only one is needed, the corresponding loss is masked, as described in Section~\ref{4.3}.
    ...
    \item \textcolor{red}{All adapter matrices and forgotten-class classifier-head nodes permanently deleted post-merge.}
\end{enumerate}
