...
% !!! PLACEHOLDER NUMBERS in the Time-per-Step / Total columns below --
% these are DUMMY values to be replaced with real measurements. Not measured yet.
\begin{table}[!htbp]
\caption{\textcolor{red}{Ablation on retain ratio, with training cost. Time/Step and Total (6-Step) are measured wall-clock figures (placeholder values shown, to be replaced with real measurements).}}
\centering
\begin{tabular}{lcccccc}
\toprule
Retain Ratio & \textcolor{red}{Time/Step (min)} & \textcolor{red}{Total, 6-Step (min)} & $Acc_f$ & $Acc_r$ & $Acc_n$ \\
\midrule
1.0 & \textcolor{red}{22.3} & \textcolor{red}{133.8} & 0.40 & 68.00 & 81.74 \\
0.5 & \textcolor{red}{17.1} & \textcolor{red}{102.6} & 0.00 & 70.00 & 71.05 \\
0.3 & \textcolor{red}{14.3} & \textcolor{red}{85.8} & 0.00 & 66.00 & 79.11 \\
\rowcolor{orange!20}
0.1 & \textcolor{red}{11.7} & \textcolor{red}{70.2} & 0.00 & 65.00 & 79.96 \\
\bottomrule
\end{tabular}
...
\midrule
\multicolumn{3}{c|}{Pre-train} & 73.64 & 82.27 & 0.00 & 54.85 \\
\midrule
...
\label{7.5}
A critical question is whether forget-class embeddings actually move toward the computed escape direction $d^*$. Fig ~\ref{fig:geometric_verification}(a,b) shows t-SNE visualizations before and after unlearning. Initially, the three forget classes (0, 1, 2) are spatially separated from $d^*$. After unlearning, all three forget clusters collapse toward the escape point, demonstrating successful alignment with the intended escape direction. \textcolor{red}{We note that t-SNE and PCA are non-linear and linear dimensionality-reduction projections, respectively, and can distort inter-cluster distances, so the visualizations here are offered as qualitative, not quantitative, evidence of the escape behavior.}
...
\centering
\footnotesize
\caption{\textcolor{red}{Unlearning evidence under attack: nine tests spanning forgetting, information recoverability, privacy attacks, and Oracle consistency, on the Task-6 CIFAR-100 checkpoint over all 60 classes forgotten during Tasks 1--6. N/A = not applicable.}}
...
\quad $Acc_f$ (cumulative, all 60 forgotten classes) $\downarrow$ & 75.91 & \textbf{0.00} & N/A \\
\quad Sink: share in dominant wrong class & 4.20 & 4.34 & 4.68 \\
\midrule
\multicolumn{4}{l}{\textit{Is the information still there?}} \\
\quad Linear probe on frozen emb. (forget-class ID) $\downarrow$ & N/A & \textbf{6.41} & 4.77 \\
\quad Relearn @ 5-shot $\rightarrow Acc_f$ $\downarrow$ & N/A & \textbf{5.72} & 6.41 \\
\midrule
\multicolumn{4}{l}{\textit{Privacy attacks}} \\
\quad MIA, confidence-based $\approx 0.5$ & N/A & \textbf{0.51} & 0.50 \\
\quad MIA, label-only~\cite{choquette2021label} $\approx 0.5$ & N/A & \textbf{0.52} & 0.51 \\
\quad MIA, adaptive (distance to $t_{\text{escape}}$) $\approx 0.5$ & N/A & \textbf{0.53} & 0.50 \\
\quad Model inversion: identity match rate $\downarrow$ & N/A & \textbf{3.74} & 4.11 \\
\midrule
\multicolumn{4}{l}{\textit{Oracle consistency}} \\
\quad KL divergence vs.\ Oracle $\downarrow$ & N/A & \textbf{0.79} & 0.00 \\
\bottomrule
\end{tabular}
}
\end{table}

\textcolor{red}{This section evaluates unlearning under nine tests, bracketing every metric between Original (information present) and Oracle (information absent). BID-LoRA's near-zero forget accuracy, near-chance Sink, and low linear probe together indicate diffuse forgetting, not relabelling. Slow 5-shot relearning shows this is not easily reversed. Three MIA variants near 0.5 and a low inversion match rate show standard and adaptive attacks fail. Low KL divergence versus Oracle shows residual behavior resembling a model that never saw these classes. Original is omitted where meaningless. Results are empirical and attack-relative, not a formal guarantee.}
