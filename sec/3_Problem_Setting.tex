...
\begin{align} 
    f_{\mathcal{M}^{(t)}} &: \mathcal{X}_{D_{f}^{(i)}} \cancel{\rightarrow} \mathcal{Y}_{D_{f}^{(i)}}, \quad \forall \, i \leq t \tag{3a} \label{Eq:3a} \\ f_{\mathcal{M}^{(t)}} &: \mathcal{X}_{D_{r}^{(t)}} \rightarrow \mathcal{Y}_{D_{r}^{(t)}} \tag{3b} \label{Eq:3b} \\ 
    f_{\mathcal{M}^{(t)}} &: \mathcal{X}_{D_{\text{new}}^{(i)}} \rightarrow \mathcal{Y}_{D_{\text{new}}^{(i)}} \tag{3c} \label{Eq:3c} 
\end{align}
...
\begin{enumerate}
    ...
    \item \textcolor{blue}{Post-merge, all adapter matrices and the pre-adaptation checkpoint are permanently deleted, making unlearning irreversible by eliminating any basis for reconstructing forgotten knowledge.}
\end{enumerate}
